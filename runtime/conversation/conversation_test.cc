 ...
 public:
  ...
  MOCK_METHOD(
      ...
      (const std::vector<absl::string_view>& target_text,
       ...
       bool store_token_lengths, (const absl::flat_hash_set<int>&)),
      (override));
  ...
  MOCK_METHOD(
      ...
      RunDecodeAsync,
      (absl::AnyInvocable<void(absl::StatusOr<Responses>)> user_callback,
       const DecodeConfig& decode_config, (const absl::flat_hash_set<int>&)),
      (override));
...
TEST_P(ConversationTest, SendSingleMessageAsync) {
  ...
  EXPECT_CALL(*mock_session_ptr,
              RunDecodeAsync(testing::_, testing::_, testing::_))
      .WillOnce(
          [](absl::AnyInvocable<void(absl::StatusOr<Responses>)> user_callback,
             const DecodeConfig& decode_config,
             const absl::flat_hash_set<int>&) {
...
TEST_P(ConversationTest, CancelGroupWithSendMessageAsync) {
  ...
  // Expect RunPrefillAsync to be called and return the first task
  // controller.
  EXPECT_CALL(*mock_session_ptr,
              RunPrefillAsync(testing::_, testing::_, testing::_))
      .WillOnce([&](const std::vector<InputData>& contents,
                   absl::AnyInvocable<void(absl::StatusOr<Responses>)>
                       user_callback,
                   const absl::flat_hash_set<int>&) {
  ...
  // Expect RunDecodeAsync to be called and return the second task
  // controller.
  EXPECT_CALL(*mock_session_ptr,
              RunDecodeAsync(testing::_, testing::_, testing::_))
      .WillOnce(
