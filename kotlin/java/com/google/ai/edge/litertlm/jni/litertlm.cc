...
LITERTLM_JNIEXPORT jlong JNICALL JNI_METHOD(nativeCreateEngine)(
    ...
    jint audio_backend_num_threads, jint engine_mode,
    jstring gpu_activation_precision, jstring gpu_cache_mode) {
  ...
  } else if (gpu_activation_precision_str == "FP16" ||
             gpu_activation_precision_str == "DEFAULT") {
    // No override required for default/FP16.
  } else {
    ThrowLiteRtLmJniException(
        env, "Unknown gpuActivationPrecision: " + gpu_activation_precision_str);
    return 0;
  }
  ...
  } else if (gpu_cache_mode_str == "DEFAULT") {
    // No override.
  } else {
    ThrowLiteRtLmJniException(env, "Unknown gpuCacheMode: " + gpu_cache_mode_str);
    return 0;
  }
